                                    ...
                                    <div class="order-progress">
                                        <ul>
                                            ...
                                            <li :class="order.orderinfo.status==2 ?'active':''">
                                                ...
                                                <div v-if="order.orderinfo.status==2" class="info">{{order.orderinfo.payment_time | dateFmt('YYYY-MM-DD HH:mm:ss')}}</div>
                                            </li>
                                            <li :class="order.orderinfo.status==3 ?'active':''">
                                                <div class="progress">已经发货</div>
                                                <div v-if="order.orderinfo.status==3" class="info">{{order.orderinfo.confirm_time | dateFmt('YYYY-MM-DD HH:mm:ss')}}</div>
                                            </li>
                                            <li class="last" :class="order.orderinfo.status==4 ?'active':''">
                                                <div class="progress">已完成</div>
                                                <div v-if="order.orderinfo.status==4" class="info">{{order.orderinfo.complete_time | dateFmt('YYYY-MM-DD HH:mm:ss')}}</div>
                                            </li>
                                        </ul>
                                    </div>
                                    <div class="form-box accept-box form-box1">
                                        <dl class="head form-group">
                                            <dd>
                                                ...
                                                <router-link v-if="order.orderinfo.status==1" :to="'/payOrder/'+order.orderinfo.id" class="btn-pay">去付款</router-link>
                                                <!---->
                                            </dd>
                                        </dl>
                                        <dl class="form-group">
            ...
            getOrderData(){
                ...
                this.$axios.get(url).then(res=>{
                    ...
                    res.data.message.goodslist.forEach(item=>{
                        tempGoodsAmount+=item.real_price * item.quantity
                    })

                    this.goodsAmount = tempGoodsAmount
                    this.payAmount = tempGoodsAmount + res.data.message.orderinfo.express_fee
